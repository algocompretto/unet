...
from utils.dataset import DriveDataset
from utils.train_evaluate import train_one_epoch, evaluate, create_lr_scheduler
...
def main(args):
    ...
    # Used to save training and verification process information
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, shuffle=True, collate_fn=train_dataset.collate_fn, **loader_args
    )

    val_loader = torch.utils.data.DataLoader(
        val_dataset, collate_fn=val_dataset.collate_fn, **loader_args
    )
    ...
    # Create a learning rate update strategy, here is updated once per step (not every epoch)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(results_file, "a") as f:
            # Record the indicators of train_loss, lr and verification set corresponding to each epoch
            train_info = (
                f"[epoch: {epoch}]\n"
    ...
    print(
        "training time {}, best_iou: {} in epoch {}.".format(
            total_time_str, str(best_iou), str(best_epoch)
        )
    )
...
def get_args():
    ...
    parser.add_argument(
        "--wd", default=5e-4, type=float, help="weight decay (default: 1e-4)"
    )
